...
void Chaplex::myLED(charlieLed led, byte state) {
	for (byte i=0; i<numberOfPins; i++) 
    	pinMode(pins[i],INPUT);
	
	if(state == 1) {
	pinMode(pins[led.r],OUTPUT);
	digitalWrite(pins[led.r], HIGH);
	
	pinMode(pins[led.c],OUTPUT);
	digitalWrite(pins[led.r]c, LOW);
	}
}
